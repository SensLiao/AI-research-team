# Council candidate — targeted repair recovery R2

> Status: `DESIGN_ONLY / NON_CITABLE / NO RESULTS`
>
> Recovery work order: `WO-TARGETED-REPAIR-R2`. This document changes only the four 3/3 replicated design defects named in `repair-packet-r2.json`; it does not reopen the ontology, data split, episode matrix, experiment identifiers, or canonical architecture.
...
### Sampling role and exact estimands
...
For an estimable target stratum (s), compute each patient's mean binary target-head log loss across that patient's `COMPLETE` episodes in (s), then

\[
\Delta^{target}_s=\operatorname{mean}_p(L^{no\_M0,target}_{p,s}-L^{full,target}_{p,s}).
\]

For an estimable scope stratum (s), analogously use the binary scope-head log loss across `SAME` episodes:

\[
\Delta^{scope}_s=\operatorname{mean}_p(L^{no\_M0,scope}_{p,s}-L^{full,scope}_{p,s}).
\]

One patient contributes one mean per stratum regardless of episode count. Descriptive 95% paired-patient bootstrap intervals use `B = 10,000`; their PCG64 seed is `20260803 XOR uint32(first_8_hex(SHA256(stratum_key)))`. Resampling carries all of a patient's episodes together. These diagnostics are not co-primary, have no pass/fail threshold, and cannot override the R1 decision. An operation-only difference, a geometry-determined pattern, or disappearance in these supported strata rejects the stronger state-relative interpretation even if the aggregate primary is positive.
...
### Pairing, seeds, and order

Retain every existing full/no_M0 experiment and replicate identifier. For each pre-existing paired row, F0 records a common `pair_id`. Define

`base_seed = uint32(first_8_hex(SHA256("T4-SM0-BLIND-01|" + pair_id)))`.

The paired arms use identical sub-seeds:

- initialization: `base_seed`;
- patient/episode sampler: `base_seed XOR 0x9E3779B9`;
- augmentation: `base_seed XOR 0x85EBCA6B`; and
- stochastic model operations: `base_seed XOR 0xC2B2AE35`.

All PRNG streams use `PCG64` where host-side NumPy sampling is used; framework RNG determinism flags and library versions are F0-manifested. The initial checkpoint is created once and loaded byte-for-byte by both arms.

Before either arm trains, generate one immutable `training-order-manifest.json` for the pair. At each optimizer update, take the next `4` distinct patients from a seeded random permutation of all training patients; when exhausted, start the next seeded permutation. For each selected patient, take the next episode from that patient's independently seeded permutation of locked eligible episodes; reshuffle only after exhausting that patient's list. One update therefore contains exactly one episode from each of four patients. Precompute and record the patient IDs, episode IDs, and augmentation parameters for all updates. Both arms consume the same rows in the same order. Ambiguity strata, M0 values, joint rarity, and outcomes never enter this sampler.

### Optimizer, budget, stopping, and checkpoint

- Optimizer: `AdamW`.
- Initial peak learning rate: `1.0e-4`.
...
- Budget: exactly `20,000` optimizer updates.
- Learning-rate schedule: linear warm-up from `0` to `1.0e-4` over updates `1..500`, then cosine decay to `1.0e-6` at update `20,000`.
...
| Repair | Replicated finding | Closed decision in this R2 document | F0 evidence still required | Truth status |
|---|---|---|---|---|
| R1 | `primary_endpoint_not_frozen` (3/3) | One exact six-legal-joint categorical log loss; equal episode/replicate averaging within patient and equal patient averaging; Δ direction fixed as no_M0 minus full; paired 10,000-draw patient bootstrap with fixed seeds and quantile algorithm; fixed `0.010`-nat margin; ordered four-state decision; strict paired completeness; one primary and no co-primary multiplicity. | Materialize and hash `analysis-contract.json`, analysis population, paired replicate set, and software numeric settings before held-out access. | Prospective design decision only; no endpoint has been computed. |
| R2 | `no_m0_neutralization_not_frozen` (3/3) | Exhaustive transitive M0 provenance closure; mandatory path classes; normalized-space exact-zero treatment; no sampler/metadata route; identical shape/capacity/init; byte-level difference allowlist; non-M0 parity; three-way M0 perturbation invariance. | Generate the real ordered 17-channel/path rows, graph attestations, neutral byte ranges, and SHA-256 for `m0-path-manifest.json` at F0. | Prospective intervention contract only; no parity test has run. |
| R3 | `ambiguity_strata_not_frozen` (3/3) | Development-only cue-visible tuple `(polarity, geometry_family, source_mode, cue_size_bin)`; exact NumPy quantile/bin algorithm; fixed distinct-patient thresholds; explicit missing/sparse handling; analysis-only role; patient-level target-on-COMPLETE and scope-on-SAME estimands. | Compute development cut points/support without held-out access, serialize exact eligible strata and counts, and hash `ambiguity-strata-manifest.json` at F0. | Prospective diagnostic contract only; no stratum result exists. |
| R4 | `curriculum_schedule_not_frozen` (3/3) | One flat patient-first schedule for both arms; deterministic pair-derived seeds; precomputed identical patient/episode/augmentation order; AdamW hyperparameters; 20,000-update budget; warm-up/cosine schedule; no early stopping; shared final update-20,000 checkpoint rule; schedule audits separate and non-rescuing. | Materialize and hash `training-schedule-manifest.json`, `training-order-manifest.json`, initial checkpoint, RNG/library record, and paired experiment mapping at F0. | Prospective training contract only; no training has occurred. |
