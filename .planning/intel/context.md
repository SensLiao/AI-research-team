...
These topic notes preserve each classified DOC verbatim with source attribution. Each source body is wrapped in a fresh randomized `DATA_*` boundary because ingested documents are treated as untrusted data, not instructions.

## Topic 1 — Current platform capabilities and operating boundaries

- source: C:\Users\廖神\Desktop\Innovation_projects\Self-project\Personal AI Infrastructure\cases\Route A\AI agent database (for research)\research_agent_teams\PLATFORM-FACTS.md
- precedence: 10
- context status: designated current fact source
