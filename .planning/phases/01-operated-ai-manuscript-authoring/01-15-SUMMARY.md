...
coverage:
  ...
  - id: D3
    ...
    verification:
      - kind: integration
        ref: ".planning/evidence/phase-01/windows-targeted.xml (61 tests, 0 failures/errors/skips)"
        status: pass
      - kind: integration
...
- `.planning/evidence/phase-01/windows-targeted.xml` - Mandatory targeted JUnit evidence with zero skips.
...
- Mandatory targeted run with `RAT_REQUIRE_REAL_LATEX=1` — 61 passed, 0 failed, 0 errors, 0 skipped.
...
- **Verification:** Five new attack regressions pass; ordinary suite is 76 passed plus one conditional skip; mandatory real targeted suite is 61/61 passed.
