...
argument-hint: "[what you want to research]"
...
> The machine never writes the vault registry — the director adds project rows there (see CLAUDE.md §2).
...
2. 把工作台指向选中的项目（只是个 pointer，不动任何数据）：
   ...
   ```
3. 路由这次请求 —— 二选一：
   - **整模式跑一遍**（7 个已接通模式之一：new_direction / gap_breadth / evidence_review / evidence_deep / deep_research / venue_readiness / full_rigor_minimal）→ 用 `/run-mode`：
     ```powershell
...
4. 没有合适项目？先 `/project-new` 建一个，或 `/project-list --include-hidden` 看归档的。
   诚实边界：full_rigor_minimal 的 EXECUTE 只产出可运行 scripts，真在 GPU 上跑要先接通服务器（CLAUDE.md §6，目前未接）。
