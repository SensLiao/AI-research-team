...
resource pool and the `.env` secret model (AGENTS.md §6). **It is part of System M (the machine), not
the database.**
...
- read-only sha256 manifests for pinned prediction / evaluation / metric files before EXECUTE
...
`primary_gpu` binding):
```bash
python -m research_agent_teams.server_monitor --live --project iac-cbct-seg --run-id <run_id>
```
`--json` emits machine-readable output (training runs collapsed to a count).

**Offline hash-manifest preview (always safe, no connection):**
```bash
python -m research_agent_teams.server_monitor.hash_manifest --iac-nnunet-nomirror --json
```

**Live read-only hash capture (director-gated, no file transfer / no remote write):**
```bash
export RAT_SERVER_QUERY_AUTHORIZED=1
python -m research_agent_teams.server_monitor.hash_manifest --iac-nnunet-nomirror --live --project iac-cbct-seg --run-id <run_id> --json
```

The captured live JSON is intended to be attached to `full_rigor_minimal` EXECUTE as
`file_identity_manifests`. Offline plan output is useful for inspection only; it is rejected by the
preflight gate as execution evidence.
...
- `research_agent_teams/server_monitor/hash_manifest.py` — read-only sha256 manifest preflight
  for dataset / prediction / metric identity checks before EXECUTE.
