...
def test_review_reconciles_all_findings_and_renders_only_from_reconciled_verdict(tmp_path):
    ...
    verdict_path = Path(next(path for path in paths if path.endswith("manuscript-review-verdict.artifact.json")))
    artifact = json.loads(verdict_path.read_text(encoding="utf-8"))
    assert validate_artifact(artifact) == []
    assert artifact["payload"]["review_run_id"] == "review-001"
