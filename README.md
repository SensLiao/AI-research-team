...
> Current status source (re-derived 2026-08-03): see `PLATFORM-FACTS.md` §0 for the full inventory
> table. Verified this checkout: **163 agents** (6 control + 157 workers) · **26 modes** (12 operated /
> 14 spec-only) · **137 tools** · **167 schemas** (167/167 parse) · **7 stages** · **5 human gates** ·
> **2 hooks** · **7 domain profiles** · **31 `operate` subcommands** · **217 test files** →
> `3914 passed, 4 skipped, exit 0`. The operated
> surface is 12 modes, the roster is 163 agents (157 scientific workers), and
...
├── schemas/      167 JSON-Schema artifact contracts (domain-agnostic; count verified by tests)
├── profiles/     7 pluggable domain profiles (cv-medical-segmentation, cs-nlp-llm, cs-rl, ai-generic, …)
├── tools/        137 deterministic validators/hashers/checkers — the enforcement primitives (incl. promote_gate)
├── tests/        217 REAL pytest files — no "passes" without terminal evidence
├── agents/       163 agent .md specs — the team roster (6 control + 157 scientific workers)
├── orchestrator/ FSM engine (engine.py) + graph spec + mode registry (26) + router + model/gate policy
├── operate/      step-wise spine + 31-verb CLI (begin / worker / run-dets / commit / brief / report / …)
│                 operate/modes/ = the 12 wired mode recipes; REGISTRY is the operated-surface truth
...
Phase 01 release evidence is accepted only when the final full-suite JUnit and
the before/after current-source SHA-256 snapshots agree; see the paired files
under `.planning/evidence/phase-01/`, with separate real-Windows-PDF, immutable-Docker-Linux, AI-evaluation,
security, route, and completion evidence under `.planning/evidence/phase-01/`.
...
- [x] **operate layer** — one-button step-wise driver + CLI (12 operated modes, including distinct `manuscript_authoring` and `manuscript_review`); **execute layer** — gated GPU runner (plan/submit/status/pull)
...
```
python -m pytest tests/ -q          # 3914 passed, 4 skipped (2026-08-03)
```

Running it from the parent directory (`python -m pytest research_agent_teams/tests/ -q`) hits three
collection errors — three manuscript test modules import `from tests....`, which needs `tests`
importable at top level. Wrong cwd, not a red suite.
