# 04 — 论文撰写路径草案（AUDITOR D 交付，2026-08-20）

**变更理由（10 行）**

1. 失效目录 E2 证实：章节/表/图顺序、agent 归属、新语料折入协议只存在于人脑中，每一步都可能漂移；导演指令要求把路径钉死并命名格式权威。
2. 失效目录 E3/E4 证实：机器有 175+ agent 却在主线程手搓了 ~15 个脚本；评审响应这一标准研究任务没有任何路由，`manuscript_reconstruction` 模式不存在。
3. 本草案把 ref-free-seg-qa 真实执行过的流水线（harvest→screen→obtain/identity→extract→deepread→synthesis→generators→prose→build→bib→dual-read→deposit）固化为 S1–S12 锁定阶段序列，每阶段一个问责席、一个唯一签收 artifact、一组停机检查。
4. 评审响应环（R1–R7）以命名子路径固化——它就是这次 Reject 返修实际走过的路（逐条核验→登记决定→机械重算→精确文字补丁→重建→盲复审），供 `manuscript_reconstruction` 模式驱动。
5. 格式权威钉在现行范本上：`Ref_Free_Seg_QA_Review_v1.0/src/type.tex`（含末尾 MONOCHROME 重定义块）+ `build.sh`（构建即体检）+ `src/main.tex`（宏文件注册表），回应 D5"版式无人拥有"。
6. 裁决语禁列表直接取自 `patch_overclaims.py` 的真实修复三元组，每条禁语配语料域替换范式，回应 D2"无裁决词表 linter"。
7. 不偏离机制逐条对应失效项：零产出通道 watchdog（A1/A6）、到货验身份（B2/B3）、独立标签（C3）、单一 loader（B1）、域限定零 + 边界记录（BR-01 教训）、折入协议（F 的 290 篇队列）。
8. 验证经济学执行导演 E1 指令：哈希只在发布时刻，凭证在 artifact 未变前持续可信，构建检查每次构建，盲评每版本一次；明令禁止逐报告重哈希与逐 build 扫色。
9. 路线寻优给出导演话型→模式→接入点映射表，并规定 `operate brief` 在请求命中时必须点名本文件；`manuscript_reconstruction` 如实标注"待注册"，不冒充已有能力（硬边界 7）。
10. 已存在的绊线（check_prose 闸、verify_corpus、synthesize.load 等）与本次升级待落地的绊线（禁语 lint、watchdog 机器化、新模式）在文中显式分列，避免把未完成后端写成在跑的功能。

---

**以下即 `docs/MANUSCRIPT-PATH-CN.md` 的完整建议全文，审阅通过后可原样拷贝。**
...
| **S5 深读抽取** | 按主题聚簇分批（`make_batches.py`），deep-reader 实例按 `corpus/EXTRACTION-SCHEMA.md` 逐篇填写抽取记录 | `corpus/text/` + `batches.json` + 抽取 schema | `corpus/deepread/<slug>.json`（每篇一条） | deep-reader 实例族（每批一签名者，`read_by` 落款） | **标签只记观察**：schema 无派生字段，任何标签不得由另一标签蕴含（阶梯式编码禁止）；词表 pinned（连字符/下划线归一）；`null` 保持 not_reported，永不折成"没做" |
| **S6 综合** | **唯一 loader** 折叠重复（`synthesize.load()`：同文多 route 记录归一）→ 证据地图、信号×功能矩阵、方法清单、gap 单；阈值来源分类（`threshold_provenance.py`）；复制/验证状态记录表（`ledger_v2.py`，只记发生过什么，不裁决成熟度）；独立多标签 recode（`recode_v2.py`）；部署成本四分（`cost_split_v2.py`）；审计链对账（`reconcile_v2.py`） | `corpus/deepread/*.json` | `corpus/synthesis/evidence-map.json`（附同目录其余综合件） | landscape-mapper | **一切分母经同一个 loader**——任何消费者（含记录表、报告）不得自行数记录；无算术裁决（"established=≥2 组+≥1 外验"这类规则禁止进生成器）；对账表能从 artifact 闭合到印刷分母 |
| **S7 生成器** | 表、图、计数宏、参考文献**全部由脚本生成**：`mktables.py`／`mktable3.py`／`mktables_v2.py`（表+宏）、`mkfig2.py`／`mkfig4.py`+`mkfig4b.py`（图）、`threshold_provenance.py`／`claims.py`（各自的表和宏；claims.py 的算术裁决规则已被 `ledger_v2.py` 的状态旗标取代，见 §2.4）、`mkbib.py`（`refs.bib` + `citekeys.json`）、`background_refs.py`（背景引用层，与证据层分离） | `corpus/synthesis/` + `corpus/deepread/` | `src/tab/*.tex` + `src/fig/*.tex` + `tab/*_numbers.tex` 宏文件全集（注册表=`src/main.tex` 头部注释：哪个生成器供给哪个宏文件） | manuscript-figure-table-engineer | **caption 与数据同一代码路径发出**（D3）；`null` 渲染 en-dash（"没说"≠"没做"）；生成器活在文件里，**禁止 heredoc 直出 LaTeX**（D6 的三次事故）；手编辑任何生成文件=违规 |
| **S8 章节撰写** | 14 个 section 文件按写作合同（参考实现：`corpus/WRITING-BRIEF.md` + `corpus/format/top-review-format-spec.md` §4 逐节字数与任务）逐席写作，integrator 收口 | 写作合同、宏文件、`citekeys.json`、deepread records | `src/sec/*.tex`（每席一文件，一个签名者） | manuscript-introduction/related-work/methods/results-author 各专席 + manuscript-section-author（其余节）+ manuscript-integrator（收口） | 语料数字**只经宏**；引用只用 `citekeys.json` 里的 citekey；语料域措辞（"在本语料中"，永不"无人做过"）；Outlook 防火墙（前瞻句只许在议程节）；裁决语禁列表扫描（§2.4）；未读论文零数字零分母 |
| **S9 构建** | `build.sh`：pass 0 = `check_numbers.py --brief`（报告）+ `check_prose.py`（**闸**：坏 citekey、未 input 的图表、悬空 cref、零值宏）→ 4-pass 编译（pdflatex→biber→pdflatex×2）→ 构建报告（未解析引用/引文、overfull 计数）→ `check_rendered.py`（渲染页检查） | `src/` 全部 | `rep/<稿名>.pdf`（附构建报告） | manuscript-integrator | **失败构建的定义**：pass 0 红、编译错误、或任何未解析 `??`——`pdflatex` exit 0 不算成功；渲染摘要词数在栏（§2.3）；`SKIP_CHECK=1` 只许调试，不得用于交付构建 |
| **S10 文献审计** | 全库对权威源审计（`ref_audit.py`：Crossref/OpenAlex/PubMed；缺陷类=preprint 已有正式版、缺 DOI、泄漏工作笔记、姓名后缀渲染、venue 不符、无法解析）；`fix_bib.py` **只施加权威记录支持的修复** | `refs.bib` + 权威 API | `corpus/logs/ref-audit.json`（附 `refs.v2.bib` 或 in-place 修复） | manuscript-citation-auditor（机器席 bibliography-validator / citation-integrity-auditor 协同） | 标题模糊匹配必须**年份一致**才接受（2017 重印顶替 U-Net 的教训）；VENUE_MISMATCH／UNRESOLVED 留人裁决，永不静默改写；XML 实体已转义；bib 有改动→回 S9 重建 |
...
| 阶段 | 干什么 | 输入 | 输出（唯一 artifact） | 负责 agent | 该停下检查什么 |
|---|---|---|---|---|---|
| **R1 评审解析** | 把评审全文拆成逐条主张清单：每条=主张原文＋涉及 artifact＋可判定证据 | 外部评审全文 | 逐条主张清单 artifact | claim-extractor | 无一条被改写、合并到失真或悄悄丢弃；不可判定的主张显式标注 |
...
| **R4 机械重算** | 新规则一律**落成生成器文件再跑**（参考实现：`recode_v2.py`／`cost_split_v2.py`／`ledger_v2.py`／`mktables_v2.py`），随后全量重跑 S7 生成器 | R3 决定 + `corpus/` | 更新后的 synthesis + tab/fig/宏全集 | manuscript-figure-table-engineer | **手改计数=违规**；caption 与数据同路径；新旧口径的差异由脚本 delta 报告证明，不由人叙述 |
| **R5 文字修复** | 精确补丁集：每处修复是 `(file, old, new)` 三元组，**必须恰好匹配一次**（可审计、可核验、可回滚，参考实现 `patch_overclaims.py`）；被重写的旧章节归档 `archive/pre-*-rebuild/` | R3 决定 + R4 宏 | 补丁日志（如 `logs/overclaim-patch.json`）＋归档 | manuscript-polish-editor | 未匹配/多匹配=硬错，绝不静默 no-op；文字修复**不得引入新数字**（数字只能来自 R4 的宏）；禁语替换按 §2.4 范式 |
